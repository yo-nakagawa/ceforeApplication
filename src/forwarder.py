...
class Publisher(Member):
    ...
    def generate(self):
        ...
        Publisher.seq += 1
        Publisher.generated_massage_repository[message_name] = str(Publisher.seq)
...
class Subscriber(Member):
    ...
    def create_packet_name(self, name):
        ...
        sequence_num = int(name_split[3])
        sequence_num = self.calculate_sequence_num(sequence_num)
        name = name_split[0] + "/"+ name_split[1] + "/" + name_split[2] + "/" + str(sequence_num)
        return name
                ...
                if len(fib_list) > 0:
                    ...
                    if info.is_succeeded:
                        ...
                        #Dataを受信した場合
                            ...
                            print(info.payload)                   
                            #受信メッセージをレポジトリへ保存
                            Subscriber.received_massage_repository[info.name] = info.payload
                            #受信メッセージの名前をGUIに表示
                            Gui.receive_message_window_update()
                            #再送タイマリセット
                            for i in range(1,3):
                                kari_name += '/'
                                kari_name += info.name.split('/')[i]
                            timeout_list[kari_name] = 10
                            #シーケンス番号インクリメントしてInterest送信    
                            packetname = self.create_packet_name(info.name)
                            #handle.send_interest(packetname,0)
                            ##各メンバに対する要求中メッセージのシーケンス番号を記録
                            Subscriber.sended_interest_repository[kari_name] = int(packetname.split('/')[3])   
                    for membername in member_list:
                        if membername != kari_name:
...
class Gui: # このクラスでは、GUIのウィジェットに関する設定
    def __init__(self):
        ...
        frame=tkinter.Frame(width=20,height=15)
        frame.place(relx=0.6,rely=0.1)
        scroll=tkinter.Scrollbar(frame)
        scroll.pack(side=tkinter.RIGHT,fill="y")
    #ラベル
        name_label = tkinter.Label(text = "Forwarder") 
        name_label.pack()
    ...
    #テキストボックス
        ...
        Role = tkinter.Entry(frame3,width = 5)
        Role.pack()
    #ボタン
        button = tkinter.Button(root,text = 'New',state="disabled",command=generate_message)
    ...
    #text-menu
        frame2=tkinter.Frame(width=20,height=15)
        frame2.place(relx=0.1,rely=0.1)
        scroll2=tkinter.Scrollbar(frame2)
        scroll2.pack(side=tkinter.RIGHT,fill="y")
        ReceiveList= tkinter.Listbox(frame2,width=20,height=15,yscrollcommand=scroll.set)
        scroll2["command"]=ReceiveList.yview 
        ReceiveList.pack()
        ReceiveList.insert(tkinter.END,"received_message")
        
        GenerateList= tkinter.Listbox(frame,width=20,height=15,yscrollcommand=scroll.set)
        scroll["command"]=GenerateList.yview
        GenerateList.pack()
        GenerateList.insert(tkinter.END,"generated_message")
        root.mainloop()
    ...
    def generate_message_window_update():
        ...
        #生成メッセージ表示ウィンドウの更新
        for message_name in Publisher.generated_massage_repository:       
            GenerateList.insert(tkinter.END,message_name)
    ...
    def receive_message_window_update():
        ...
        scroll2=tkinter.Scrollbar(frame2)
        scroll2.pack(side=tkinter.RIGHT,fill="y")
        ReceiveList= tkinter.Listbox(frame2,width=20,height=15,yscrollcommand=scroll2.set)
        scroll2["command"]=ReceiveList.yview        
        ReceiveList.pack()
        ReceiveList.insert(tkinter.END,"received_message")
        
        #受信メッセージ表示ウィンドウの更新
        for message in Subscriber.received_massage_repository:       
            ReceiveList.insert(tkinter.END,message)
    def role_bar_update(str1):
        frame3 = tkinter.Frame(width = 5)
        frame3.place(relx=0.46,rely=0.1)
        Role = tkinter.Entry(frame3,width = 5)
        Role.pack()
        Role.insert(tkinter.END,str1)
...
def fib_update(interface_name):
    ...
    for nodename, addr in neighbor_list.items(): #リンクダウンしたと思われるネイバーはリスト破棄
        if addr[1] == "":
            if nodename == "/FR2": continue
            if len(fib_list) == 0 or len(fib_list) == 1: return
    ...
    for i in range(proc_list_num):
        for nodename, addr in neighbor_list.items():
            if proc_list[i].split(' ')[4] == addr[0]: #リンク上にいるやつ
                ...
                if proc_list[i].split(" ")[5] != "REACHABLE" and proc_list[i].split(" ")[5] != "DELAY" and proc_list[i].split(" ")[5] != "STALE":
...
def wifi_setteing():
    ...
    while True:
        ...
        for index,iface in enumerate(interface_list):                
            ...
            if iface == "wlo1": subprocess.run(['/usr/local/sbin/wpa_supplicant','-Dnl80211','-i'+ iface,'-c/etc/wpa_supplicant/p2p1.conf','-B'])
            else: subprocess.run(['/usr/local/sbin/wpa_supplicant','-Dnl80211','-i'+ iface,'-c/etc/wpa_supplicant/p2p.conf','-B'])      
            you_go = False
            print("----------------------------")           
            ...
            while True:
                ...
                for kari in kari_list: #発見されたネイバーを片っ端から、前方一致検索
                    ...
                    if iface == "wlx18ece77d737f":
                        memnam = "/FR1"
                    else:
                        memnam = "/FR2"
                    if peer_name == memnam:
                        if peer_name in neighbor_list.keys():
                            continue
                        print(peer_name + " is found !")
                        if kari.split(",")[1].split("=")[1] == "0x9":   you_go = True
                        if peer_macaddr == "70:9c:d1:21:b5:b1":
                            neighbor_list[peer_name] = ["70:9c:d1:21:b5:b2",""]
                        else:
                            neighbor_list[peer_name] = [peer_macaddr,""]   #発見されたネイバーを追加(IPはまだ)
                        break
                else:
                    sleep(1)
                    continue
            ...
            if you_go:
                ...
                subprocess.run(["/usr/local/sbin/wpa_cli", "p2p_connect",'-i'+ iface,peer_macaddr,"pbc"])
            sleep(5)
            if len(netifaces.interfaces()) - 3  > len(interface_list): #Intelのとき,この2はloとイーサカードを抜いて計算
...
def link_layer_administration(): #10秒usr/local/sbin/wpaごとに、ICMPv6 Echo Requestを投げる
    global my_macaddr
    global my_ipv6addr
...
if __name__ == '__main__':
    ...
    #仮想無線インターフェイスを記録
    for iface in  netifaces.interfaces():
        if iface != 'lo' and not iface.startswith('enp') and not iface.startswith('docker0'): interface_list.append(iface)
